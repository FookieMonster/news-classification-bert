...
<dl>
  ...
  <dd>モデルサーバに予測リクエストを送る前に、前処理を行うだけのプロキシーです。AI Platform Predictionにはカスタム予測ルーチンという仕組みで前処理をモデルサーバー側に含ませることができるのですが、カスタム予測ルーチンはモデルサイズ500MB以下の制限があるので今回は前処理を行うだけのPython環境を設置しました。</dd>
</dl>
